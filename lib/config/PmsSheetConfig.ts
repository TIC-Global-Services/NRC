...
export async function fetchPortfolioData(): Promise<PortfolioRow[]> {
    const url = process.env.NEXT_PUBLIC_PMS_SHEET_URL;

    if (!url) {
        throw new Error("Missing NEXT_PUBLIC_HOME_SHEET_URL in .env.local");
    }
    ...
    const formatted: PortfolioRow[] = raw
        .map((row: any, index: number) => {
            ...
            // Get benchmark value - looking for "S&P BSE 500 Index" column
            ...
            // Get portfolio value - looking for "Aurum Multiplier Portfolio" column
